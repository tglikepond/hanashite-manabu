...
6. 일본어 번역(japanese), 예문(example), 반대 스타일(alt_japanese) 등 모든 일본어 텍스트에서 한자가 포함된 경우 반드시 한자 바로 옆에 괄호로 히라가나/가타카나 읽기를 표기하세요
    - 예: "食(た)べてみる", "行(い)かなければならない", "彼女(かのじょ)は学生(がくせい)です"
    - 한자 하나하나마다 괄호로 읽기를 붙이세요
7. 각 표현에 실제 사용 가능한 일본어 예문을 포함하고, 예문의 한국어 번역(example_korean)도 반드시 포함하세요
...
13. 2개 이상의 단어가 조합된 표현이나 문장의 경우, word_composition 필드에 어떤 단어들이 합쳐진 것인지 구성 분석을 제공하세요
    ...
    - 예: "行(い)かなければならない" → "行(い)く(가다) + なければならない(~해야 한다) = 가야 한다"
  ...
  "expressions": [
    {
      ...
      "word_composition": "2개 이상 단어 조합인 경우 구성 분석 (예: 食(た)べる(먹다) + てみる(~해보다) = 먹어보다), 단일 단어면 빈 문자열",
