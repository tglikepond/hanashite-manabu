...
중요 규칙:
1. 대화를 분석한 뒤 두 가지 타입으로 분류하세요:
   - type "sentence": 문장, 구문, 관용표현 (2개 이상의 단어가 조합된 표현). 5~8개 선별
   - type "word": 개별 단어 (명사, 동사, 형용사, 부사 등 한 단어). 5~10개 선별
2. 단어(word)에는 다음을 모두 포함하세요:
   - 대화에서 직접 사용된 핵심 단어
   - 위에서 선별한 문장(sentence) 표현을 구성하는 주요 단어들
   - 단어의 기본형(사전형)으로 정리하세요
3. 모든 문장을 번역하지 마세요. 중복되거나 사소한 표현은 제외하세요
4. 각 표현의 중요도를 판단하세요:
   - "필수": 일상 대화에서 매우 자주 쓰이며 반드시 알아야 하는 표현
   - "자주사용": 자연스러운 대화를 위해 알아두면 좋은 표현
   - "유용": 특정 상황에서 유용한 표현
5. 한국어 표현의 어감을 감지하세요:
   - 반말이면 detected_style을 "반말", 존댓말이면 "존댓말"로 설정
   - 단어(word) 타입은 detected_style을 "기본형"으로 설정
6. 메인 번역은 감지된 스타일로, 대안 번역은 반대 스타일로 제공
   - 단어(word) 타입은 alt_japanese/alt_reading/alt_pronunciation을 빈 문자열로 두세요
7. 일본어 번역(japanese), 예문(example), 반대 스타일(alt_japanese) 등 모든 일본어 텍스트에서 한자가 포함된 경우 반드시 한자 바로 옆에 괄호로 히라가나/가타카나 읽기를 표기하세요
    - 예: "食(た)べてみる", "行(い)かなければならない", "彼女(かのじょ)は学生(がくせい)です"
    - 한자 하나하나마다 괄호로 읽기를 붙이세요
8. 각 표현에 실제 사용 가능한 일본어 예문을 포함하고, 예문의 한국어 번역(example_korean)도 반드시 포함하세요
9. 대화 전체의 맥락을 요약하고, 학습 팁도 제공하세요
10. 해설은 한국어로 작성
11. 발음 표기는 로마자(romaji)가 아닌 한글로 작성하세요 (예: "오하요 고자이마스")
12. 예문의 발음도 한글로 표기하세요
13. 반대 스타일 번역의 발음도 한글로 표기하세요
14. 문장(sentence) 타입의 경우, word_composition 필드에 어떤 단어들이 합쳐진 것인지 구성 분석을 제공하세요
    - 예: "食(た)べてみる" → "食(た)べる(먹다) + てみる(~해보다) = 먹어보다"
    - 단어(word) 타입은 word_composition을 빈 문자열로 두세요
...
반드시 아래 JSON 형식으로만 응답하세요:
{
  ...
  "expressions": [
    {
      "type": "sentence 또는 word",
      "korean": "원래 한국어 표현",
      "importance": "필수" 또는 "자주사용" 또는 "유용",
      "detected_style": "반말" 또는 "존댓말" 또는 "기본형",
      "japanese": "메인 일본어 번역 (한자에 읽기 괄호 포함)",
      "reading": "히라가나 읽기",
      "pronunciation": "한글 발음 표기",
      "alt_style": "반대 스타일 이름 (word 타입이면 빈 문자열)",
      "alt_japanese": "반대 스타일 번역 (word 타입이면 빈 문자열)",
      "alt_reading": "반대 스타일 히라가나 (word 타입이면 빈 문자열)",
      "alt_pronunciation": "반대 스타일 한글 발음 (word 타입이면 빈 문자열)",
      "explanation": "간단한 문법/표현 해설 (한국어)",
      "word_composition": "sentence 타입: 구성 분석, word 타입: 빈 문자열",
      "example": "이 표현을 활용한 자연스러운 일본어 예문 1개 (한자에 읽기 괄호 포함)",
      "example_korean": "예문의 한국어 번역",
  ...
  card.innerHTML = `
    ...
    <div class="card-detail">
      ...
      <div class="card-example">
        ...
        ${expr.example_korean ? `<div class="example-kr">${escHtml(expr.example_korean)}</div>` : ''}
  ...
  if (isSaved) {
    ...
    const doSave = async (e) => {
      ...
      const exprToSave = {
        ...
        example_korean: expr.example_korean || '',
...
function renderStudyPaper(result) {
  ...
  const allExprs = result.expressions || [];
  const sentences = allExprs.filter(e => e.type === 'sentence');
  const words = allExprs.filter(e => e.type === 'word');
  // Fallback: if no type field (backward compat), treat all as sentences
  if (sentences.length === 0 && words.length === 0 && allExprs.length > 0) {
    sentences.push(...allExprs);
  }

  // Render sentence section
  if (sentences.length > 0) {
    const sectionEl = createTypeSection('💬', '문장 · 구문', sentences.length, 'sentence');
    ...
    sorted.forEach(expr => listEl.appendChild(renderExpressionCard(expr, false)));
    els.resultsList.appendChild(sectionEl);
  }
...
function createTypeSection(icon, title, count, typeClass) {
  const section = document.createElement('div');
  section.className = `note-group type-${typeClass}`;
  section.innerHTML = `
    <div class="note-group-header">
      <div class="note-group-header-left">
        <span class="note-group-icon">${icon}</span>
        <span class="note-group-title">${title}</span>
    ...
    <div class="type-section-list note-group-list"></div>
  `;
  const header = section.querySelector('.note-group-header');
  ...
  if (expressions.length === 0) {
    ...
    if (query || (sentences.length === 0 && words.length === 0)) {
      ...
      if (allSentences.length > 0) {
        ...
        const listEl = sectionEl.querySelector('.type-section-list');
        allSentences.forEach(expr => listEl.appendChild(renderExpressionCard(expr, true)));
